...
class GauzeRetrieveFullDof_haptic(PsmEnv):
    """
    ...
    def __init__(self, render_mode=None, cid = -1):
        super(GauzeRetrieveFullDof_haptic, self).__init__(render_mode, cid)
    ...
    def _env_setup(self):
        np.random.seed(2048)
        super(GauzeRetrieveFullDof_haptic, self)._env_setup()
        ...
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'gauze/gauze.urdf'),
                            (2.71023607e+00, -4.28840667e-02,  3.46959066e+00),
                            (0, 0, 0, 1),
                            useFixedBase=False,
                            globalScaling=self.SCALING)
