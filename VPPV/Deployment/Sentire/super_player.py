...
class VisPlayer(nn.Module):
    ...
    def init_run(self):
        ...
        self.p.read_pose_twist_jaw()
        ecm_pos = self.p.pose.p
        ecm_rot = self.p.pose.M
        tip_pose_psm = np.array([[ecm_rot[0,0], ecm_rot[0,1], ecm_rot[0,2], ecm_pos[0]],
                                [ecm_rot[1,0], ecm_rot[1,1], ecm_rot[1,2], ecm_pos[1]],
                                [ecm_rot[2,0], ecm_rot[2,1], ecm_rot[2,2], ecm_pos[2]],
                                [0,0,0,1]])

        psm_pose = self.p.measured_cp()
        psm_pos = psm_pose.p
        psm_rot = psm_pose.M
        rcm_pose_psm = np.array([[psm_rot[0,0], psm_rot[0,1], psm_rot[0,2], psm_pos[0]],
                                [psm_rot[1,0], psm_rot[1,1], psm_rot[1,2], psm_pos[1]],
                                [psm_rot[2,0], psm_rot[2,1], psm_rot[2,2], psm_pos[2]],
                                [0,0,0,1]])           
        # tip_rcm = ECMtip_T_cam @ tip_pose_psm @ np.linalg.inv(rcm_pose_psm)
        tip_rcm = tip_pose_psm @ np.linalg.inv(rcm_pose_psm)    
        self.cam_T_basePSM = tip_rcm
        self.basePSM_T_cam = np.linalg.inv(tip_rcm)
    ...
    def check_finished(self, curr_robot_pos,depth ,seg):
        ...
        if self.is_success(curr_robot_pos, self.rcm_goal) or self.count>self.player_opts.max_step[self.task]:
            ...
            if self.task=='vessel':
                self.manipulator.vessel_clip(self.object_point, self.depth_img, seg, step_num, tool_offset, self.basePSM_T_cam)
            elif self.task=='gauze':
                self.manipulator.grasp_tissue(self.object_point,self.depth_img, seg, step_num, tool_offset, self.basePSM_T_cam)
                self.manipulator.place_tissue(self.place_point, self.depth_img, seg, step_num, tool_offset, self.basePSM_T_cam)
                ...
                self.manipulator.needle_pick(self.object_point, self.depth_img, seg, step_num, tool_offset, self.basePSM_T_cam)
...
if __name__=="__main__":
    ...
    parser.add_argument('--task', required=True, type=str, help="choose from 'gauze', 'vessel','tissue','needle'")
