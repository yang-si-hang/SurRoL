...
def plot_depth(img, path='/home/student/code/IGEV/IGEV-Stereo/demo-output', name='test2.png'):
...
def demo(args):
    ...
    with torch.no_grad():
        ...
        for (imfile1, imfile2) in tqdm(list(zip(left_images, right_images))):
            ...
            filename = os.path.join(output_directory, f"{file_stem}2.png")
            #plt.imsave(output_directory / f"{file_stem}.png", disp.squeeze(), cmap='jet')
            #disp = cv2.normalize(dis, None, 0, 1, cv2.NORM_MINMAX) #0,1
