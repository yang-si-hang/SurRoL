...
from .utils.visualizer import Visualizer, read_video_from_path
from .predictor import CoTrackerPredictor

class CoTrackerVisualSurrolAPI:

    def __init__(self):
        self.model = CoTrackerPredictor(checkpoint='/home/kejianshi/Desktop/Surgical_Robot/science_robotics/RealRobot_code/animal_code/co-tracker/cotracker/checkpoints/cotracker2.pth')
        self.device = 'cuda'
    ...
    def step(self, img, point=None):
        ...
        if len(self.video_st) == 1:
            for _ in range(5):
                self.video_st.append(img)
                
        video = torch.from_numpy(np.array(self.video_st)).permute(0, 3, 1, 2)[None].float()
