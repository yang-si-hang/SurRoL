...
def add_pixel_flip_noise(seg_image, p=0.3):
    """
    Randomly flip pixels in segmentation image with probability p
    Args:
        seg_image: Input segmentation image (numpy array)
        p: Probability of flipping each pixel (default: 0.3)
    Returns:
        Noisy segmentation image with flipped pixels
    """
    # Create random mask where True represents pixels to flip
    flip_mask = np.random.random(seg_image.shape) < p  # p probability for each pixel
    
    # Create noisy image by flipping selected pixels
    noisy_image = seg_image.copy()
    noisy_image[flip_mask] = abs(1 - noisy_image[flip_mask])  # Flip from 0->1 or 1->0
    
    return noisy_image


def add_gaussian_noise(image, mean=0, sigma=0.001):
    """
    Add Gaussian noise to image
    Args:
        image: Input image (numpy array)
        sigma: Standard deviation of Gaussian noise (default: 0.001)
    Returns:
...
def add_seg_MarkovNoise_random(gt, T, theta1, theta2, theta3):
    '''
        Generate the Markov noise, 2D version
    '''
