...
class Sampler:
    ...
    def sample_action(self, obs, is_train, v_action=False):
        return self._agent.get_action(obs, noise=is_train, v_action=v_action)
    
    def sample_episode(self, is_train, render=False, random_act=False, v_action=False):
        """Samples one episode from the environment."""
        self.init()
        episode, done = [], False
        while not done and self._episode_step < self._max_episode_len:
            #print('1')
            action = self._env.action_space.sample() if random_act else self.sample_action(self._obs, is_train, v_action=v_action) 
            if action is None:
                break
            if render:
