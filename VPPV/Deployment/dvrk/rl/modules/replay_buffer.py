...
class HerReplayBuffer:
    ...
    def __init__(self, env_params, buffer_size, batch_size, sampler, T=None):
        ...
        self.buffers = {'obs': np.empty([self.size, self.T + 1, self.env_params['obs']]),
                        #'obs': torch.empty(self.size, self.T+1,self.env_params['obs'] ),
                        ...
                        'v': np.empty([self.size, self.T+1, 2, self.img_size,self.img_size])
                        }
    
    def store_episode(self, episode_batch):
        '''Store (multiple) episodes to the buffer'''
        mb_obs, mb_ag, mb_g, mb_actions, dones, v = episode_batch.obs, episode_batch.ag, episode_batch.g, \
                                                    episode_batch.actions, episode_batch.dones, episode_batch.v
        #if not isinstance(mb_obs, torch.Tensor):
        #    mb_obs=torch.tensor(mb_obs)
        batch_size = mb_obs.shape[0]
        idxs = self._get_storage_idx(inc=batch_size)

        # store the informations
        #print('replay buffer, m_obs: ',mb_obs.shape)
        #print(self.buffers['obs'][idxs].shape)
        #print(mb_obs)

        self.buffers['obs'][idxs] = mb_obs.copy()
        self.buffers['ag'][idxs] = mb_ag.copy()
        self.buffers['g'][idxs] = mb_g.copy()
        self.buffers['actions'][idxs] = mb_actions.copy()
        self.buffers['dones'][idxs] = dones.copy()
        self.buffers['v'][idxs]= v.copy()
    ...
    def sample(self):
        ...
        temp_buffers['obs_next'] = temp_buffers['obs'][:, 1:, :].copy()
        temp_buffers['ag_next'] = temp_buffers['ag'][:, 1:, :].copy()
        # sample transitions
    ...
    def save(self, save_dir, episode):
        ...
        #with open(os.path.join(save_dir, f"replay_buffer_ep{episode}.pkl"), 'wb') as f:
        #    pickle.dump(self.buffers, f)
...
class HERSamplerSeq(HERSampler):
    ...
    def sample_her_transitions(self, episode_batch, batch_size_in_transitions):
        ...
        #transitions = {key: episode_batch[key][episode_idxs, t_samples].copy() for key in episode_batch.keys() if (key!='obs' and key!='obs_next') }
        #transitions['obs']=episode_batch['obs'][episode_idxs, t_samples]
        #transitions['obs_next']=episode_batch['obs_next'][episode_idxs, t_samples]
        transitions = {key: episode_batch[key][episode_idxs, t_samples].copy() for key in episode_batch.keys()}
        #print(transitions['obs'].shape) #1,3,19
        #print(transitions['obs_next'].shape) # 2 19
        #print(transitions['ag'].shape) # 2,3
        ...
        future_ag = episode_batch['ag'][episode_idxs[her_indexes], future_t]
        transitions['g'][her_indexes] = future_ag
        #print(transitions['ag_next'].shape)
