...
class VisDEX(DDPGBC):
    def __init__(
        ...
        self.v_processor=VisProcess(vo_dim=self.vo_dim)
    ...
    def get_samples(self, replay_buffer):
        ...
        # For vis
        seg_d=self.to_torch(transitions['v'])
        ...
        with torch.no_grad():
            ...
            if self.obj_num>1:
                for i in range(1, self.obj_num):
                    pos=robot_state[:3]+rel_pos[i*self.obj_num:3*self.obj_num]
    ...
    def update_vprocesser(self, seg_d, v_gt):
        ...
        #print(obs_demo[:9])
        #print(output.shape)
        #print(obs_demo.shape)
        v_loss=0.
        #v_loss=F.mse_loss(output,v_gt, reduction='sum')
        pos_loss=F.mse_loss(output[:3*self.obj_num],v_gt[:3*self.obj_num])
        v_loss+=pos_loss
        pose_loss=F.mse_loss(output[3*self.obj_num:],v_gt[3*self.obj_num:])
        v_loss+=pose_loss
    ...
    def update(self, replay_buffer, demo_buffer):
        ...
        for i in range(self.update_epoch):
            ...
            obs, action, reward, done, next_obs, _ , seg_d, v_gt= self.get_samples(replay_buffer)
