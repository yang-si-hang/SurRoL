...
from general_utils import flatten_dict, np2obj, prefix_dict
from vis_utils import add_captions_to_seq
...
class WandBLogger:
    ...
    def log_outputs(self, logging_stats, rollout_storage, log_images, step, is_train=False, log_videos=True, log_video_caption=False, video_name="rollouts"):
            ...
            if 'image' in rollout_storage and log_videos:
                if log_video_caption:
                    ...
                    vids = [np.stack(rollout.image).transpose(0, 3, 1, 2)
                            for rollout in rollout_storage.get()[-self.n_logged_samples:]]
                self.log_videos(vids, name=video_name, step=step)
...
class Logger(object):
    def __init__(self, log_dir):
        ...
        self._train_mg = MetersGroup(f'{log_dir}/train.csv',
                                     formating=COMMON_TRAIN_FORMAT)
        self._eval_mg = MetersGroup(f'{log_dir}/eval.csv',
