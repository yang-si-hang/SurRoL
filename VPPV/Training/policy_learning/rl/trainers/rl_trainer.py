...
class RLTrainer(BaseTrainer):
    ...
    def _setup_misc(self):
        #init_buffer(self.cfg, self.buffer, self.agent, normalize=False, view_matrix=self.train_env.get_camera_matrix()) # important for awac, amp
        # TODO: update demonstration
        #init_buffer(self.cfg, self.demo_buffer, self.agent, normalize=True, view_matrix=self.train_env.ecm.view_matrix)
    ...
    def train(self):
        ...
        #print('work: ',self.cfg.mpi.num_workers)
        ...
        while train_until_episode(self.global_episode):
            ...
            if not self.cfg.dont_save and save_every_episodes(self.global_episode) and self.is_chef:
                ...
                #self.buffer.save(self.buffer_dir, self.global_episode)
    ...
    def eval_v(self):
        ...
        for ii in range(self.cfg.n_eval_episodes):
            ...
            eval_rollout_storage.append(episode)
            episode_v, _, env_steps_v = self.train_sampler.sample_episode(is_train=False, render=True, v_action=True, v_model=self.v_model)
        ...
        if self.is_chef:
            if self.cfg.use_wb:
                self.wb.log_outputs(rollout_status, eval_rollout_storage, log_images=True, step=self.global_step)
                self.wb.log_outputs(rollout_status_v, eval_rollout_storage_v, log_images=True, step=self.global_step,video_name="v_obs")
        ...
        return rollout_status_v.avg_success_rate_v, rollout_status.avg_success_rate
    ...
    def resume_training(self):
        ...
        # print('self.agent.g_norm_mean', self.agent.g_norm_mean)
        # torch.save(self.agent.state_dict(),'needle_policy.pt')
        # # self.agent.train()
        # #self.agent.eval()
        # #self.agent.v_processor.train()
        # exit()
        
            
        '''
        if self.cfg.reload_vis:
            env_params2= AttrDict(
            #obs=obs['observation'].shape[0],
            #achieved_goal=obs['achieved_goal'].shape[0],
            #goal=obs['desired_goal'].shape[0],
                obs=self.cfg.agent.obs_num,
                achieved_goal=self.cfg.agent.achieved_goal_num,
                goal=self.cfg.agent.goal_num,
                act=7,
                act_rand_sampler=self.train_env.action_space.sample,
                max_timesteps=self.train_env._max_episode_steps,
                max_action=self.train_env.action_space.high[0],
            )
            agent_2 = make_agent(env_params2, self.buffer_sampler, self.cfg.agent)
            agent_2.cuda()

            CheckpointHandler.load_checkpoint(
                    self.cfg.vis_ckpt_dir, agent_2, self.device, "best"
                )

            self.agent.v_processor=agent_2.v_processor
        '''
        if self.cfg.test:
            self.agent.eval()
            #self.v_model.eval()
            sr = self.eval()
            #self.termlog.info(f'Vision Successful rate: {avg_success_rate_v}')
            self.termlog.info(f'Successful rate: {sr}')
            
            exit()

        if self.cfg.restart_vis:
            self.agent._restart_vis_processor()
        #if not self.cfg.buffer_dir is None:
        #    self.buffer.load(self.cfg.buffer_dir, 1600)
