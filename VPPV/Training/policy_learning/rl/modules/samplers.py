...
class Sampler:
    ...
    def sample_action(self, obs, is_train, v_action=False, v_model=None):
        return self._agent.get_action(obs, noise=is_train, v_action=v_action, v_model=v_model)
    
    def sample_episode(self, is_train, render=False, random_act=False, v_action=False, v_model=None):
        """Samples one episode from the environment."""
        self.init()
        episode, done = [], False
        while not done and self._episode_step < self._max_episode_len:
            action = self._env.action_space.sample() if random_act else self.sample_action(self._obs, is_train, v_action, v_model) 
            if action is None:
                break
            if render:
                render_obs,_, _=self._env.ecm.render_image()
                #render_obs = self._env.render('rgb_array')
            #print("save debug!")
            #cv2.imwrite('/research/d1/rshr/arlin/SAM-rbt-sim2real/exp_local/debug/posdebugrgb.png',cv2.cvtColor(render_obs, cv2.COLOR_BGR2RGB))
            #exit()
