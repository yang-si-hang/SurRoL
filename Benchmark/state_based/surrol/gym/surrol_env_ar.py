...
class SurRoLEnv(gym.Env):
    ...
    def __init__(self, render_mode: str = None, cid: int = -1):
        ...
        #self.action_space = spaces.Box(-0.0000001, 0.0000001, shape=(self.action_size,), dtype='float32')
        # TODO : modify the setting for observation space init. Currently only for needle pick
        #self.obs_shape={'achieved_goal': (3,), 'observation':(19,)}
        
        obs = self._get_obs()
        #print('-->surrol_env.init: ', obs['observation'].requires_grad)
        if isinstance(obs, np.ndarray):
            ...
            desired_goal=spaces.Box(-np.inf, np.inf, shape=obs['achieved_goal'].shape, dtype='float32'),
                ...
                seg_d=spaces.Box(-np.inf, np.inf, shape=obs['seg_d'].shape, dtype='float32')
        ))
        else:
    ...
    def render(self, mode='rgb_array'):
        ...
        rgb_array, mask, _ = render_image(RENDER_WIDTH, RENDER_HEIGHT,
