...
from rl.components.envrionment import MatchBoardSLWrapper
...
def main():
    ...
    env = MatchBoardSLWrapper(env, output_raw_obs=True, subtask=args.subtask)
...
def goToGoal(env, last_obs_, last_obs):
    ...
    while time_step < min(env.max_episode_steps, args.steps):
        ...
        if i == SUBTASK_END[args.subtask]:
            ...
            if args.subtask in ['pull, push', 'grasp']:
                action[-1] = -0.5
            else:
                action[-1] = 0.5
