...
class MatchBoardPanel(PsmEnv):
    ...
    def _env_setup(self):
        super(MatchBoardPanel, self)._env_setup()
        ...
        # match board
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'match_board_panel_rl/match_board.urdf'),
        ...
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'match_board_panel_rl/' + file_name),
                                np.array(pos),
    ...
    def get_oracle_action(self, obs) -> np.ndarray:
        ...
        for i, waypoint in enumerate(self._waypoints):
            ...
            scale_factor = 0.85 if i not in [3, 14] else 0.85
            # scale_factor = 0.55
...
if __name__ == "__main__":
    env = MatchBoardPanelRL(render_mode='human')  # create one process and corresponding env
