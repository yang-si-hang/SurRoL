...
from surrol.tasks.psm_env_RL_2 import PsmsEnv, goal_distance
...
class BiPegBoard(PsmsEnv):
    ...
    def _env_setup(self):
        ...
        # board
        asset_scaling = 0.185
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, "peg_board_bimanual/ring_board.urdf"),
        ...
        # peg 
        obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, "peg_board_bimanual/ring_peg.urdf"),
                            np.array(self.POSE_BOARD[0]) * self.SCALING,
                            p.getQuaternionFromEuler(self.POSE_BOARD[1]),
                            globalScaling=asset_scaling,
                            useFixedBase=1)
        ...
        np.random.shuffle(self._pegs[2:5])
        self.obj_ids['fixed'].append(obj_id)

        # rings
        num_rings = 3
        for i in self._pegs[2: 2 + num_rings]:
            pos, orn = get_link_pose(self.obj_ids['fixed'][-1], i)
            obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, "ring_peg_board_bimanual/ring.urdf"),
        ...
        # set psm rotation
        self.psm1_eul = np.deg2rad([-15, 0, -180])
        self.psm2_eul = np.array([np.deg2rad(-90), 0., 0])
    ...
    def _sample_goal(self) -> np.ndarray:
        ...
        goal = np.array([peg_pos[0], peg_pos[1], peg_pos[2] - 0.01 * self.SCALING])
        #goal = np.array(get_link_pose(self.obj_id, self.obj_link1)[0])
        return goal.copy()
    ...
    def _sample_goal_callback(self):
        ...
        offset_y = 0.009 * self.SCALING + (0.0000) * self.SCALING * (self._pegs[2] - 2)
        ...
        self._waypoints.append(np.array([nsd_pos_mid1[0], nsd_pos_mid1[1] + 0.02 * self.SCALING, nsd_pos_mid1[2], yaw1, -0.5,
                                         nsd_pos_mid2[0], nsd_pos_mid2[1], nsd_pos_mid2[2], yaw2, 0.5]))   # psm1 move 8

        #----------------------------Subtask 3----------------------------
        target_pos = self.goal.copy()
        yaw1 = np.pi / 2.1
        offset_y = 0.017 * self.SCALING  # offset casued by rotation
        self._waypoints.append(np.array([target_pos[0], target_pos[1] + offset_y, target_pos[2] + 0.03 * self.SCALING, yaw1, -0.5,
                                         nsd_pos_mid2[0], nsd_pos_mid2[1], nsd_pos_mid2[2], yaw2, 0.5]))  # psm1 above target 9
        ...
        self._waypoints.append(np.array([target_pos[0], target_pos[1] + offset_y, target_pos[2] + 0.02 * self.SCALING, yaw1, 0.5,
                                         nsd_pos_mid2[0], nsd_pos_mid2[1], nsd_pos_mid2[2], yaw2, 0.5]))  # psm1 release 11
