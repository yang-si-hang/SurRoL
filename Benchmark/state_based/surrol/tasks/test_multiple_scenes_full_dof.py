...
def open_scene(id):
    ...
    if id == 0:
        ...
        scene = SurgicalSimulator(NeedlePickFullDof, {'render_mode': 'human'},id)
    elif id == 2:
...
class SurgicalSimulator(SurgicalSimulatorBase):
    ...
    def before_simulation_step(self):
        ...
        retrived_action = np.array([0, 0, 0, 0, 0, 0, 0], dtype = np.float32)
        getDeviceAction_right(retrived_action)

        # haptic right
        # retrived_action-> x,y,z, angle, buttonState(0,1,2)
        if retrived_action[6] == 2:
            self.psm1_action[0] = 0
            self.psm1_action[1] = 0
            self.psm1_action[2] = 0
            self.psm1_action[3] = 0
            self.psm1_action[4] = 0
            self.psm1_action[5] = 0
         
        else:
            self.psm1_action[0] = retrived_action[2]*0.75
            self.psm1_action[1] = retrived_action[0]*0.75
            self.psm1_action[2] = retrived_action[1]*0.75
            # self.psm1_action[0] = 0
            # self.psm1_action[1] = 0
            # self.psm1_action[2] = 0            
            self.psm1_action[3] = -retrived_action[4]/math.pi*180*1.5 #pitch
            self.psm1_action[4] = 0
            self.psm1_action[5] = 0
            # self.psm1_action[3] = 0
            self.psm1_action[4] = -retrived_action[5]/math.pi*180*0.75 #yaw
            self.psm1_action[5] = retrived_action[3]/math.pi*180*0.75 #roll
        if retrived_action[6] == 0:
            self.psm1_action[6] = 1
        if retrived_action[6] == 1:
            self.psm1_action[6] = -0.5
