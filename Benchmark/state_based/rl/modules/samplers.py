...
class RLIFSampler(Sampler):
    def __init__(self, env, agent, max_episode_len):
        super().__init__(env, agent, max_episode_len)
    ...
    def sample_episode(self, is_train, render=False, random_act=False):
        ...
        while not done and self._episode_step < self._max_episode_len:
            ...
            # TODO @Jiawei: Add expert policy to check Q value here to determine intervene probability

            intervene = np.random.choice([0, 1], p=[0.99, 0.01])
            if intervene:
                action = self._env.get_oracle_action(self._obs)
