...
class RLIF(BaseAgent):
    ...
    def update(self, replay_buffer, demo_buffer):
        ...
        for i in range(self.update_epoch):
            ...
            obs_demo, action_demo, reward_demo, done_demo, next_obs_demo = self.get_samples(demo_buffer)

            # print(f'obs shape: {obs.shape}')
            # print(f'obs_demo shape: {obs_demo.shape}')
            # print(f'action shape: {action.shape}')
            # print(f'action_demo shape: {action_demo.shape}')
            # print(f'reward shape: {reward.shape}')
            # print(f'reward_demo shape: {reward_demo.shape}')
            # print(f'next_obs shape: {next_obs.shape}')
            # print(f'next_obs_demo shape: {next_obs_demo.shape}')

            # print(f'reward_demo: {reward_demo}')

            obs = torch.cat([obs, obs_demo], dim=0)
            action = torch.cat([action, action_demo], dim=0)
            reward = torch.cat([reward, reward_demo], dim=0)
            next_obs = torch.cat([next_obs, next_obs_demo], dim=0)
