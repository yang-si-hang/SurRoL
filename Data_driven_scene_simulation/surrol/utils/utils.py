...
class Boundary(object):
    ...
    def _get_position_within_boundary(self) -> tuple:
        x = np.random.uniform(self._boundary[0][0], self._boundary[0][1])
        y = np.random.uniform(self._boundary[1][0], self._boundary[1][1])
        z = np.random.uniform(self._boundary[2][0], self._boundary[2][1])
        return x, y, z
    ...
    def set_boundary(self, boundary: [list, tuple, np.ndarray]):
        ...
        self._area = float(np.prod(self._boundary[:, 1] -
                                   self._boundary[:, 0]))
    ...
    def add(self,
        ...
        if not sample:
            ...
            rotation = np.random.uniform(list(min_rotation),
                                         list(max_rotation))
...
class Trajectory(object):
    ...
    def generate_trajectory(self, num_points):
        ...
        boundary_x = np.random.uniform(
            self.workspace_limits[0, :].mean() -
            0.5 * self.workspace_limits[0, :].ptp(),
            self.workspace_limits[0, :].mean() -
            0.3 * self.workspace_limits[0, :].ptp())
        boundary_y = np.random.uniform(
            self.workspace_limits[1, :].mean() -
            0.1 * self.workspace_limits[1, :].ptp(),
            self.workspace_limits[1, :].mean() +
            0.1 * self.workspace_limits[1, :].ptp())
        # upper left
        pts_ul = np.random.uniform(low=(self.workspace_limits[0,
                                                              0], boundary_y),
                                   high=(boundary_x, self.workspace_limits[1,
                                                                           1]),
                                   size=(3, 2))
        pts_ul = pts_ul[pts_ul[:, 1].argsort()[::-1]]  # by y, descend
        # bottom left
        pts_bl = np.random.uniform(low=(self.workspace_limits[0, 0],
                                        self.workspace_limits[1, 0]),
                                   high=(boundary_x, boundary_y),
                                   size=(3, 2))
        pts_bl = pts_bl[pts_bl[:, 1].argsort()[::-1]]  # by y, descend
        # bottom right
        pts_br = np.random.uniform(low=(boundary_x, self.workspace_limits[1,
                                                                          0]),
                                   high=(self.workspace_limits[0,
                                                               1], boundary_y),
                                   size=(3, 2))
        pts_br = pts_br[pts_br[:, 0].argsort()]  # by x, ascend
        # upper left
        pts_ur = np.random.uniform(low=(boundary_x, boundary_y),
                                   high=(self.workspace_limits[0, 1],
                                         self.workspace_limits[1, 1]),
                                   size=(3, 2))
        pts_ur = pts_ur[pts_ur[:, 0].argsort()[::-1]]  # by x, ascend
        pts = np.concatenate(
            [pts_ul, pts_bl, pts_br, pts_ur, pts_ul[0].reshape((1, 2))],
            axis=0)
        self.pts = pts
...
def get_centroid(mask: np.ndarray, target: int) -> Tuple[bool, np.ndarray]:
    ...
    if np.sum(target_mask > 0):
        ...
        cX = M["m10"] / M["m00"]
        cY = M["m01"] / M["m00"]
        normalized_x = (cX - img_shape[1] / 2) / (img_shape[1] -
                                                  img_shape[1] / 2)
        normalized_y = (cY - img_shape[0] / 2) / (img_shape[0] -
                                                  img_shape[0] / 2)
...
def inverse_gamma(x, gamma=1.0):
    return 255.0 * (x / 255.0)**gamma


def adjust_gamma(x, gamma=1.0):
    invGamma = 1.0 / gamma
    return (x / 255.0)**invGamma * 255