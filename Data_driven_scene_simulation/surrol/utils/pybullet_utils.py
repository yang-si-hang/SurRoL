...
ConstraintInfo = namedtuple('ConstraintInfo', [
    'parentBodyUniqueId', 'parentJointIndex', 'childBodyUniqueId',
    'childLinkIndex', 'constraintType', 'jointAxis', 'jointPivotInParent',
    'jointPivotInChild', 'jointFrameOrientationParent',
    'jointFrameOrientationChild', 'maxAppliedForce'
])
...
JointState = namedtuple('JointState', [
    'jointPosition', 'jointVelocity', 'jointReactionForces',
...
def violates_limits(body, joints, values):
    return any(
        violates_limit(body, joint, value)
        for joint, value in zip(joints, values))
...
def get_difference_fn(body, joints):
    def fn(q2, q1):
        ...
        for joint, value2, value1 in zip(joints, q2, q1):
            difference.append(
                circular_difference(value2, value1) if is_circular(
                    body, joint) else (np.array(value2) - np.array(value1)))
...
def get_extend_fn(body, joints, resolutions=None):
    ...
    def fn(q1, q2):
        ...
        for i in range(num_steps):
            waypoints.append(
                list(((float(i) + 1.0) / float(num_steps)) * np.array(diffs) +
                     q1))
        return waypoints
...
def pairwise_link_collision(body1,
                            link1,
                            body2,
                            link2,
    ...
    return p.getContactPoints(bodyA=body1,
                              bodyB=body2,
                              linkIndexA=link1,
                              linkIndexB=link2)
...
def get_moving_pairs(body, moving_joints):
    ...
    for i in range(len(moving_links)):
        ...
        for j in range(i + 1, len(moving_links)):
            ...
            ancestors2 = set(get_joint_ancestors(body,
                                                 link2)) & set(moving_joints)
...
def get_collision_fn(body, joints, obstacles, attachments, self_collisions,
                     disabled_collisions):
...
def dump_body(body):
    ...
    for joint in get_joints(body):
        print('Joint id: {} | Name: {} | Type: {} | Circular: {} | Limits: {}'.
    ...
    print('Link id: {} | Name: {} | Mass: {}'.format(-1, get_base_name(body),
                                                     get_mass(body)))
    for link in get_links(body):
        print('Link id: {} | Name: {} | Parent: {} | Mass: {}'.format(
            link, get_link_name(body, link),
...
def forward_kinematics(body, joints=None, positions=None, eef_link=None):
    ...
    """
    eef_link = get_num_joints(
        body) - 1 if eef_link is None else eef_link  # not true for dVRK models
...
LinkState = namedtuple('LinkState', [
    'linkWorldPosition', 'linkWorldOrientation', 'localInertialFramePosition',
    'localInertialFrameOrientation', 'worldLinkFramePosition',
...
def get_com_pose(body, link):  # COM = center of mass
    ...
    return list(link_state.linkWorldPosition), list(
        link_state.linkWorldOrientation)
...
def get_movable_joint_ancestors(body, link):
    return list(
        filter(lambda j: is_movable(body, j), get_joint_ancestors(body, link)))
...
def get_adjacent_fixed_links(body):
    return list(
        filter(lambda item: not is_movable(body, item[0]),
...
DynamicsInfo = namedtuple('DynamicsInfo', [
    'mass', 'lateral_friction', 'local_inertia_diagonal', 'local_inertial_pos',
    'local_inertial_orn', 'restitution', 'rolling_friction',
    'spinning_friction', 'contact_damping', 'contact_stiffness'
])
...
def render_image(width, height, view_matrix, proj_matrix, shadow=1):
    (_, _, px, _,
     mask) = p.getCameraImage(width=width,
                              height=height,
