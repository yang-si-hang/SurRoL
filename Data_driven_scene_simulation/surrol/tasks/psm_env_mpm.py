...
from surrol.utils.myprint import print_c
...
class PsmEnvMPM(SurRoLGoalEnv):
    ...
    def __init__(self, render_mode=None, cid=-1):
        ...
        self.particles_list = []
        self.meshes_list = []
        self.colormap = plt.get_cmap("seismic")
        self.rendering_cost = []
        self.rigid_sim_cost = []
        self.soft_sim_cost = []
        # workspace
        workspace_limits = np.asarray(self.WORKSPACE_LIMITS1) + np.array(
            [0.0, 0.0, 0.0102]
        ).reshape(
            (3, 1)
        )  # tip-eef offset with collision margin
        workspace_limits *= (
            self.SCALING
        )  # use scaling for more stable collision simulation
        ...
        self.obj_id, self.obj_link1, self.obj_link2 = (
            None,
            None,
            None,
        )  # obj_link: waypoint link

        # gripper
        self.block_gripper = True
        self._activated = -1
        self._max_constraint_id = 1
        super(PsmEnvMPM, self).__init__(render_mode, cid)
    ...
    def _env_setup(self, goal_plot=True):
        ...
        self.psm1 = Psm1(
            self.POSE_PSM1[0],
            p.getQuaternionFromEuler(self.POSE_PSM1[1]),
            scaling=self.SCALING,
        )
        self.psm1_eul = np.array(
            p.getEulerFromQuaternion(
                self.psm1.pose_rcm2world(self.psm1.get_current_position(), "tuple")[1]
            )
        )  # in the world frame
        if self.ACTION_MODE == "yaw":
            self.psm1_eul = np.array([np.deg2rad(-90), 0.0, self.psm1_eul[2]])
    ...
    def _get_robot_state(self, idx: int) -> np.ndarray:
        ...
        return np.concatenate(
            [
                np.array(pose_world[0]),
                np.array(p.getEulerFromQuaternion(pose_world[1])),
                np.array(jaw_angle).ravel(),
            ]
        )  # 3 + 3 + 1 = 7
    ...
    def _get_obs(self) -> dict:
        ...
        if self.has_object:
            ...
            achieved_goal = (
                object_pos.copy() if not self._waypoint_goal else waypoint_pos.copy()
            )
        else:
            # tip position
            achieved_goal = np.array(
                get_link_pose(self.psm1.body, self.psm1.TIP_LINK_INDEX)[0]
            )
    ...
    def _set_action(self, action: np.ndarray):
        ...
        ), "The action should have the save dim with the ACTION_SIZE"
        # time0 = time.time()
        action = (
            action.copy()
        ...
        pose_world[:3, 3] = np.clip(
            ...
            workspace_limits[:, 0] - [0.02, 0.02, 0.0],
            workspace_limits[:, 1] + [0.02, 0.02, 0.08],
        )  # clip to ensure convergence
        ...
        if self.ACTION_MODE == "yaw":
            ...
            rot = (
                self.psm1_eul[0],
                self.psm1_eul[1],
                wrap_angle(rot[2] + action[3]),
            )  # only change yaw
        elif self.ACTION_MODE == "pitch":
            action[3] *= np.deg2rad(15)  # pitch, limit maximum change in rotation
            pitch = np.clip(
                wrap_angle(rot[1] + action[3]), np.deg2rad(-90), np.deg2rad(90)
            )
    ...
    def _step_callback(self, demo=1):
        ...
        if self._contact_constraint is None:
            ...
            if self._meet_contact_constraint_requirement():
                ...
                points_1 = [
                    point[2] for point in points_1 if point[2] in self.obj_ids["rigid"]
                ]
                points_2 = [
                    point[2] for point in points_2 if point[2] in self.obj_ids["rigid"]
                ]
        ...
        if self._activated < 0:
            ...
            if self._contact_approx:
                ...
                if not self._waypoint_goal:
                    ...
                    link_id = (
                        self.obj_link1 if idx == 0 else self.obj_link2
                    )  # TODO: check
                pos_obj, _ = get_link_pose(self.obj_id, link_id)
                if (
                    np.linalg.norm(np.array(pos_tip) - np.array(pos_obj))
                    < 2e-3 * self.SCALING
                ):
                    self._activated = idx
                    # disable collision
                    p.setCollisionFilterPair(
                        bodyUniqueIdA=psm.body,
                        bodyUniqueIdB=self.obj_id,
                        linkIndexA=6,
                        linkIndexB=-1,
                        enableCollision=0,
                    )
    ...
    def get_soft_body_goal_position(self, idx, soft_bpos):
        if self.mpm_solver is None:
            return None
        else:
            return np.array(self.mpm_solver.F_x[idx]) + soft_bpos
    ...
    def soft_body_step(
        ...
        t1 = time.time()
        if use_debug:
            print_c(f"MPM step time: {(t1-t0)*1000}ms", color="yellow")
        ...
        if draw_soft_body:
            if USE_POINTS:
                if gs_model is not None:
                    # particles_array = gs_model.get_xyz.cpu().detach().numpy()+ base_position
                    particles_array = self.mpm_solver.F_x.to_numpy()*scale + base_position
                else:
                    particles_array = self.mpm_solver.F_x.to_numpy()*scale + base_position

                num_particles = min(10000,len(particles_array))
                particles_array = particles_array[np.random.choice(np.array(range(len(particles_array))), num_particles)]
                idx = 0
                ...
                # scene.particles(self.mpm_solver.F_x, radius=0.0005, color=(0.934, 0.33, 0.23))

                # vtx_deformation = np.zeros(shape=(len(vtx),),dtype=np.float32)
                # vtx_velocity = np.zeros(shape = (len(vtx),3), dtype=np.float32)
                # vtx_force = np.zeros(shape = (len(vtx),3), dtype=np.float32)
                # for i, v in enumerate(vtx):
                #     base = v.astype("int")
                #     dx, dy, dz = v - base
                #     if visualize_way == "deformation":
                #         vtx_deformation[i] = self.mpm_solver.interpolate_deformation(
                #             base, dx, dy, dz
                #         )
                #     elif visualize_way == "velocity":
                #         vtx_velocity[i] = self.mpm_solver.interpolate_velocity_field(base, dx, dy, dz)
                #     elif visualize_way == "force":
                #         vtx_force[i] = self.mpm_solver.interpolate_force_field(base, dx, dy, dz)
                # vtx = vtx / (self.mpm_solver.n_grid * 1.0)
                # vertices = ti.Vector.field(3, float, len(vtx))
                # vertices.from_numpy(vtx)
                # print(
                #     f"max deformation: {vtx_deformation.max()}, min deformation: {vtx_deformation.min()}"
                # )
                # deform_max, deform_min = vtx_deformation.max(), vtx_deformation.min()
                # vtx_deformation = (vtx_deformation + 0.5) #/ 2.0
                # vtx_colors = self.colormap(vtx_deformation).astype(np.float32)
                # colors = ti.Vector.field(3, float, len(vtx))
                # if visualize_way == "deformation":
                #     colors.from_numpy(vtx_colors)
                # elif visualize_way == "velocity":
                #     colors_array = plt.get_cmap('YlGnBu')(vtx_velocity[:,2]*0.3).astype(np.float32)
                #     colors.from_numpy(colors_array)
                # elif visualize_way == "force":
                #     colors.from_numpy(vtx_force)
                # indices = ti.field(int, len(idx))
                # indices.from_numpy(idx)
                ...
                if texture_id != -1:
                    ...
                    UV = vtx[:, :2]*3-anchor[:2]
                    #TODO: deformation based texture mapping?
                    self.vid = p.createVisualShape(
                ...
                body_id = p.createMultiBody(
                    baseVisualShapeIndex=self.vid, baseCollisionShapeIndex=cid
                )
                if texture_id != -1:  #!: may lead to crash in v2
                    p.changeVisualShape(
                ...
                t4 = time.time()
                if use_debug:
                    print_c(f"Rendering time: {(t4-t3)*1000}ms", color="yellow")
    ...
    def sim_step(
        ...
        t1 = time.time()
        if debug:
            print_c(f"Whole Soft Simulation:{(t1-t0)*1000}ms", color="red")
    ...
    def save_ply(self, path):
        self.mpm_solver.save_ply(path)


class PsmsEnvMPM(PsmEnvMPM):
    """
    ...
    POSE_TABLE = ((0.5, 0, 0.001), (0, 0, 0))
    WORKSPACE_LIMITS1 = ((0.50, 0.60), (-0.0, 0.05), (0.675, 0.745))
    WORKSPACE_LIMITS2 = ((0.50, 0.60), (-0.05, 0.0), (0.675, 0.745))
    SCALING = 1.0

    def __init__(self, render_mode=None, cid=-1):
        # workspace
        workspace_limits = np.asarray(self.WORKSPACE_LIMITS2) + np.array(
            [0.0, 0.0, 0.0102]
        ).reshape(
            (3, 1)
        )  # tip-eef offset with collision margin
        ...
        super(PsmsEnvMPM, self).__init__(render_mode, cid)
    ...
    def _env_setup(self):
        ...
        if self.ACTION_MODE == "yaw":
            self.psm2_eul = np.array([np.deg2rad(-90), 0.0, self.psm2_eul[2]])
        elif self.ACTION_MODE == "pitch":
            self.psm2_eul = np.array(
                [np.deg2rad(-180), self.psm2_eul[1], np.deg2rad(90)]
            )
        else:
            raise NotImplementedError
    ...
    def _get_obs(self) -> dict:
        ...
        if self.has_object:
            ...
            object_pos = waypoint_pos1 = waypoint_rot1 = waypoint_pos2 = (
                waypoint_rot2
            ) = object_rel_pos1 = object_rel_pos2 = np.zeros(0)

        if self.has_object:
            achieved_goal = (
                object_pos.copy() if not self._waypoint_goal else waypoint_pos1.copy()
            )
        else:
            # tip position
        ...
        observation = np.concatenate(
            [
                robot_state,
                object_pos.ravel(),
                object_rel_pos1.ravel(),
                object_rel_pos2.ravel(),
                waypoint_pos1.ravel(),
                waypoint_rot1.ravel(),
                waypoint_pos2.ravel(),
                waypoint_rot2.ravel(),  # achieved_goal.copy(),
            ]
        )
        obs = {
            "observation": observation.copy(),
    ...
    def _set_action(self, action: np.ndarray):
        ...
        *2 for PSM1 [0: 5] and PSM2 [5: 10]
        ...
        for i, psm in enumerate((self.psm1, self.psm2)):
            ...
            idx = i * 5
            workspace_limits = (
                self.workspace_limits1 if i == 0 else self.workspace_limits2
            )
            pose_world[:3, 3] = np.clip(
                pose_world[:3, 3] + action[idx : idx + 3],
                workspace_limits[:, 0] - [0.02, 0.02, 0.0],
                workspace_limits[:, 1] + [0.02, 0.02, 0.08],
            )  # clip to ensure convergence
            rot = get_euler_from_matrix(pose_world[:3, :3])
