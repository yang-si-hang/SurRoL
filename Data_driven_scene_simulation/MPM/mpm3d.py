...
class MPM_Solver:
    ...
    def initialize_from_gs_model(self, gs_model, MAX_COLLISION_OBJECTS=3):
    ...
    def init_kernel(self, n_particles, MAX_COLLISION_OBJECTS=3, gravity = 0.0, p_rho =1000):
        ...
        self.particle_mask = ti.field(int, n_particles)
        self.particle_mask.fill(0)
        self.particle_opacity = ti.field(float, n_particles)

        self.F_grid_v = ti.Vector.field(dim, float, shape=grid_shape)
        self.F_grid_force = ti.Vector.field(dim, float, shape=grid_shape)
        self.F_grid_m = ti.field(float, grid_shape)
        # self.visual_m = ti.field(float, (256,256,256))
    ...
    @ti.kernel
    def select(self, point: ti.template(), scale: ti.template()):
        for p in self.F_x:
            dx = self.F_x[p] - point
            if dx.norm_sqr() < scale:
                self.particle_mask[p] = 1
    ...
    def set_parameters(self, s_E=8000, s_nu=0.2, eta=0.5):
    ...
    def P2G(self):
        for p in self.F_x:
            ...
            stress += self.eta[None] * (self.F_C[p] + self.F_C[p].transpose()) #viscosity
            stress = (-self.dt * self.p_vol * 4 * self.inv_dx * self.inv_dx) * stress

            principal_stress = ti.Vector([.0,.0,.0])
            _, sig, _ = ti.svd(stress)
            for i in ti.static(range(self.dim)):
                principal_stress[i] = sig[i, i]
            ...
            for offset in ti.static(ti.grouped(ti.ndrange(*self.neighbour))):
                ...
                if (
                    ...
                    self.F_grid_force[base + offset] += weight * principal_stress
    ...
    #                 for i in ti.static(range(self.dim)):
    #                     weight *= w[offset[i]][i]
    #                 self.visual_m[base + offset] += weight * self.p_mass
    ...
    def Boundary(self, scale, threshold=0.05):
        for I in ti.grouped(self.F_grid_m):
            ...
            if open_collision_handling and SDF[I] < threshold and self.F_grid_m[I] > 0:
                ...
                gdx = (
                    (SDF[i + 1, j, k] - SDF[i - 1, j, k])
                    * 0.5
                    * self.inv_dx
                    * inv_scale
                )
                gdy = (
                    (SDF[i, j + 1, k] - SDF[i, j - 1, k])
                    * 0.5
                    * self.inv_dx
                    * inv_scale
                )
                gdz = (
                    (SDF[i, j, k + 1] - SDF[i, j, k - 1])
                    * 0.5
                    * self.inv_dx
                    * inv_scale
                )
                ...
                collision_object_velocity = self.co_v[tag]
                ...
                # sticky trick
                self.F_grid_v[I] = collision_object_velocity/scale
            else:
                collision_mask[I] = -1
    ...
    def G2P(self):
        for p in self.F_x:
            ...
            # if self.particle_mask[p] == 1:
            #     self.F_v[p] = self.particle_velocity
    ...
    @ti.kernel
    def pos_generation(self, scale: float):
        for i in ti.grouped(self.pos):
            self.pos[i] = i * self.dx * scale

    def init_pos(self,scale):
        self.pos_generation(scale)
    ...
    def interpolate_deformation(self, base, dx, dy, dz):
        ...
        for v in range(8):
            ...
            if self.F_grid_m[idx] > 0.0:
                deformation += w * self.F_grid_deformation[idx] * self.p_mass / self.F_grid_m[idx]
        return deformation
    
    def interpolate_velocity_field(self, base, dx, dy, dz):
        res = ti.Vector([.0,.0,.0])
        base = tuple(base)
        x, y, z = base
        for v in range(8):
            ...
            w *= (1 - k) + (2 * k - 1) * dz
            if self.F_grid_m[idx] > 0.0:
                res += w * self.F_grid_v[idx]
        return res
    
    def interpolate_force_field(self, base, dx, dy, dz):
        res = ti.Vector([.0,.0,.0])
        base = tuple(base)
        x, y, z = base
        for v in range(8):
            ...
            w *= (1 - k) + (2 * k - 1) * dz
            if self.F_grid_m[idx] > 0.0:
                res += w * self.F_grid_force[idx]
        return res
    
    @ti.kernel
    def deformation_visualization(self, vtx:ti.template(), deformation:ti.template()):
        for v in vtx:
            base = int(vtx[v])
            dx = vtx[v] - base
            for m in ti.static(range(8)):
                i, j, k = m & 1, m >> 1 & 1, m >> 2 & 1
                idx = int(base + ti.Vector([i,j,k]))
                w = 1.0
                w *= (1 - i) + (2 * i - 1) * dx[0]
                w *= (1 - j) + (2 * j - 1) * dx[1]
                w *= (1 - k) + (2 * k - 1) * dx[2]
                if self.F_grid_m[idx] > 0.0:
                    deformation[v] += (self.F_grid_deformation[idx] / self.F_grid_m[idx]) * w
    ...
    def init_cube(self):
        ...
        for i in range(self.n_particles):
            ...
            self.F_x[i] = ti.Vector(
                [ti.random() * 0.2, ti.random() * 0.2, ti.random() * 0.05]
            )
    ...
    def init_model_from_ply(self, filename):
        ...
        # pts[:,2]-=0.1 #for better stomach placement
    ...
    def reset(self, backups: dict):
        self.set_xyz(backups['xyz'])
        self.set_cov(backups['cov'])
        self.set_normalize_parameter(backups['normalize_base'], backups['normalize_scale'])
        self.F_v.fill([0, 0, 0])
    ...
    def update_J(self):
        ...
        for p in self.FJ:
            ...
            for offset in ti.static(ti.grouped(ti.ndrange(*self.neighbour))):
                ...
                self.F_grid_deformation[base + offset] += weight * (self.FJ[p] - 1)
    ...
    def step(
        ...
        sdf.switch_reference_frame_and_update_sdf(
            i_rot_list=inverse_rot_list, i_pos_list=inverse_pos_list, co_obj=self.co_obj
        )
        ...
        co_end = time.time()
        if debug:
            print_c(f"Collision detection using {(co_end-co_begain)*1000}ms", color="green")
        ...
        s_e = time.time()
        if debug:
            print_c(f"Pure soft body simulation used {(s_e-s_b)*1000.0}ms", color="green")
    ...
    def get_mesh(self, smooth_scale=0.2, debug=False):
        ...
        if debug:
            print_c(f"GPU/CPU Communication time: {(t1-t0)*1000}ms", color = "green")
        ...
        if debug:
            print_c(f"Marching Cubes time: {(t1-t0)*1000}ms", color = "green")
    ...
    def save_ply(self, save_path):
        ...
        writer.export_frame_ascii(self.frame, save_path)
