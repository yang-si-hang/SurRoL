...
class MPM_Simulator_WARP:
    ...
    def initialize(self, n_particles, n_grid=100, grid_lim=1.0, device="cuda:0"):
        ...
        self.mpm_model.update_cov_with_F = True
        ...
        self.mpm_state.grid_opacity = wp.zeros(
            shape=(self.mpm_model.n_grid, self.mpm_model.n_grid, self.mpm_model.n_grid),
            dtype=float,
            device=device,
        )
    ...
    def load_initial_data_from_torch(
        ...
        self.import_particle_x_from_torch(tensor_x, clone=True, device=device)
        if tensor_opacity is not None:
            self.import_particle_opacity_from_torch(tensor_opacity, clone=True, device=device)
    ...
    def compute_opacity_field(self, device= "cuda:0"):
        wp.launch(
            kernel=compute_grid_opacity,
            dim=self.n_particles,
            inputs=[self.mpm_state,self.mpm_model],
            device=device
        )
    
    def step(self, steps, dt):
    ...
    def import_particle_opacity_from_torch(self, tensor_opacity, clone=True, device="cuda:0"):
        if tensor_opacity is not None:
            if clone:
                tensor_opacity = tensor_opacity.clone().detach()
            self.mpm_state.particle_opacity = torch2warp_float(tensor_opacity, dvc=device)
    ...
    def export_opacity_field_to_torch(self):
        return wp.to_torch(self.mpm_state.grid_opacity)
    ...
    def add_impulse_on_particles(self, force, dt, point =[1,1,1], size = [1,1,1], num_dt = 1, start_time=0.0, device = "cuda:0"):
        ...
        num= wp.to_torch(impulse_param.mask).sum()
        print(f"Selected Particles Number:{num}")
        
        if num==0:
            print("\033[91mParticles number is zero!!!\033[0m")
            # sys.exit(-1)
